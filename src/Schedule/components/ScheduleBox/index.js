...
const ScheduleBox = ({ switchStarIcon, item }) => {
  return (
    <View style={styles.scheduleBox}>
      ...
      <View style={styles.scheduleBoxSideTime}>
        <Text style={styles.sideTimeText}>
          {item.startTime}
          {'\n'}
          {item.startZone}
        </Text>
        ...
        <TouchableWithoutFeedback onPress={() => switchStarIcon(item)}>
          <View>
            ...
            {item.isStarred === false && (
              <Ionicons name="ios-star-outline" size={30} color="black" />
            )}
          </View>
        </TouchableWithoutFeedback>
      </View>
      <Text style={styles.scheduleBoxDetail}>{item.location}</Text>
      <Text style={styles.scheduleBoxDetail}>
        {item.startTime} {item.startZone} - {item.endTime} {item.endZone}
      </Text>
    </View>
  );
};
