...
const ScheduleScreen = (props) => {
  ...
  function renderFlatList(item) {
    if (item.isStarred) {
      return <ScheduleBox switchStarIcon={switchStarIcon} item={item} />;
    }
    return null;
  }
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.agendaTabs}>
        <TouchableOpacity
          style={styles.showAgenda}
          onPress={() => toggleAgenda(true)}
        >
          {agenda && <View style={styles.agendaTabColor} />}
          <Text style={styles.agendaTabText}>Agenda</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.showMyAgenda}
          onPress={() => toggleAgenda(false)}
        >
          {!agenda && <View style={styles.agendaTabColor} />}
      ...
      <CalendarTab
        exampleScheduleList={exampleScheduleList}
        date={date}
